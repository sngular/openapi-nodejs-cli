...
export const writeOutputFile = (
	...
	const source = fs.readFileSync(path.join(__dirname, `/../templates/${templateName}.hbs`), 'utf-8');
	...
	log(
		`Writing ${templateName} output file on ${path.join(process.cwd(), outputDir)} as ${filename}.${extension}`,
		templateName
	);
	fs.writeFileSync(path.join(process.cwd(), `/${outputDir}/${filename}.${extension}`), output, 'utf-8');
};
