...
export function parseDocument(data: DataObject) {
	...
	if (data.components) {
		...
		componentsKeys.forEach((key: string) => {
			data.components[key] = parseSchemas({ [key]: data.components[key] }, data)[0];
		});
	}
	...
	data.paths.forEach(({ methods }: DataObject, pathIndex: number) => {
		methods.forEach(({ parameters, requestBody, responses }: DataObject, methodIndex: number) => {
			// Parse method parameters if exists
			if (parameters) {
				parameters.forEach((parameter: DataObject, parameterIndex: number) => {
					data.paths[pathIndex].methods[methodIndex].parameters[parameterIndex].type = parseTypes(parameter.schema);
				});
			}
			...
			if (requestBody) {
				if (requestBody.content && requestBody.content['application/json']) {
					data.paths[pathIndex].methods[methodIndex].requestBody.content['application/json'].type = parseTypes(
			...
			if (responses) {
				...
				data.paths[pathIndex].methods[methodIndex].responses = parseResponses(responses);
				...
				responses.forEach(({ content }: DataObject, responseIndex: number) => {
					if (content && content['application/json']) {
						...
						data.paths[pathIndex].methods[methodIndex].responses[responseIndex].content['application/json'].type =
							parseTypes(content['application/json'][typeKeys[0]]);
					}
				});
			}
		});
	...
	newSchemas.forEach((schema) => {
		if (schema.allOf) {
			schema.allOf.forEach((item: DataObject, index: number) => {
				if (item['$ref']) {
					let refSchema = data;
					resolveSchemaName(item['$ref']).forEach((item) => (refSchema = refSchema[item]));
	...
	Object.keys(properties).forEach((property) => {
		if (properties[property].enum) {
			...
			properties[property].enum.forEach((element: string, index: number) => {
				enumType =
					index === properties[property].enum.length - 1 ? enumType + `'${element}'` : enumType + `'${element}' | `;
			});
			...
			if (properties[property].items['$ref']) {
				itemType = resolveSchemaName(properties[property].items['$ref']).slice(-1)[0];
			}
