...
export async function getSpecificationFiles(pathsToSpec: string[]): Promise<DataObject> {
	let data: DataObject = {};
	...
	for (const pathToSpec of pathsToSpec) {
		...
		if (!jsonData.components) {
				...
				methods.forEach((method: string) => {
					...
					statusCodes.forEach((statusCode: string) => {
						if (responses[statusCode].content) {
							const contentKeys = Object.keys(responses[statusCode].content['application/json']);
							contentKeys.forEach((key: string) => {
								responses[statusCode].content['application/json'][key] = {
									$ref: parseTypes(responses[statusCode].content['application/json'][key]),
								};
							});
						}
					});
			...
			if (refMatch !== null) {
				const componentsPath: string | null = getComponentPath(refMatch[0], pathToSpec);

				if (componentsPath) {
					const response = await getComponentsFiles(componentsPath, isUrl(componentsPath));
					let componentsData: DataObject = {};

					const fileExtension = componentsPath.split('.').slice(-1)[0].toLowerCase();
