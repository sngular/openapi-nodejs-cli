...
export function getUsedComponents(data: DataObject): string[] {
	...
	data.paths.forEach((path: DataObject) => {
		path.methods.forEach((method: DataObject) => {
			if (method.responses) {
				method.responses
					.filter((response: DataObject) => response.content && response.content['application/json'])
					.forEach((response: DataObject) => {
						const parsedTypes = parseTypes(response.content['application/json']);
						if (parsedTypes) {
...
export function cleanComponents(components: DataObject, usedComponents: string[]): DataObject {
